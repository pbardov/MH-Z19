...
float MHZ19::getTemperatureOffset(bool force) {
  ...
  if (errorCode == RESULT_OK || force == false) {
    /* Value appears to be for CO2 offset from 24C (useful for deriving CO2 from
     * raw?) */

    float calc = (((responseTEMPUNLIM[2] - 8) * 1500) +
                  ((responseTEMPUNLIM[3] * 100) * 1 / 17));
    calc /= 100;
    return calc;
  }
...
void MHZ19::stablise() {
...
#ifdef ESP32
      ESP_LOGE(TAG_MHZ19,
               "Failed to verify connection(1) to sensor. Failed to stablise");
#else
      Serial.println("!ERROR: Failed to verify connection(1) to sensor. Failed "
                     "to stablise");
#endif
...
#ifdef ESP32
      ESP_LOGE(TAG_MHZ19,
               "Failed to verify connection(2) to sensor. Failed to stablise");
#else
      Serial.println("!ERROR: Failed to verify connection(2) to sensor. Failed "
                     "to stablise");
#endif
  ...
  for (byte i = 2; i < 8; i++) {
    if (responseTEMPUNLIM[i] != responseSTAT[i]) {
      ...
      ESP_LOGE(
          TAG_MHZ19,
          "Last response was not found, call back failed. Failed to stablise");
#else
      Serial.println("!ERROR: Last response was not found, call back failed. "
                     "Failed to stablise");
#endif
...
void MHZ19::calibrateZero(int rangeCal) {
  if (rangeCal) {
    int rangevalues[11] = {400,  1000, 1500, 2000, 3000, 4000,
...
void MHZ19::recoveryReset() { provisioning(RECOVER); }
...
void MHZ19::constructCommand(Command_Type commandtype, int inData) {
  ...
  /* set address to 'any' */
  asemblecommand[0] =
      255; ///(0xFF) 255/FF means 'any' address (where the sensor is located)
...
void MHZ19::handleResponse(Command_Type commandtype) {
  if (constructedCommand[2] == Commands[2]) // compare commands byte
    receiveResponse(
        responseRAW,
        commandtype); // returns errornum, passes back response, inputs command
...
byte MHZ19::receiveResponse(byte inBytes[9], Command_Type commandnumber) {
  ...
  if (inBytes[0] != constructedCommand[0] ||
      inBytes[1] != constructedCommand[2])

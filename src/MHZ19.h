...
public:
  ...
  /* request CO2 values, 2 types of CO2 can be returned, isLimted = true
   * (command 134) and is Limited = false (command 133) */
  ...
  /* returns Raw CO2 value as a % of transmittance */ //<--- needs work to
                                                      // understand
  ...
  /* Returns what appears to be an offset */ //<----- knowledgable people might
                                             // have success the raw value
  ...
  /* returns temperature using command 163 (Note: this library deducts -2 from
   * getTemperature() as it is incorrect) */
...
private:
  ...
  /* Incoming Data Holders */ // Important to be aware of if utalising force
                              // arguments
  byte responseTEMPUNLIM[9];  // Holds command 133 response values "temperature
                              // unlimited"
  byte responseTEMPLIM[9];    // Holds command 134 response values "temperature
                              // limited"
  byte responseRAW[9];        // Holds command 132 response values "CO2 Raw"
  byte responseSTAT[9]; // Holds other command response values such as range,
                        // background CO2 etc
